...
## 실행 컨텍스트에 대해 말해보세요
execute context 란 소스코드를 실행하는 데 필요한 환경을 제공하고 코드의 실행 결과를 실제로 관리하는 영역입니다.  

chrom 개발자도구로 Call Stack 에 가보면 실행 컨텍스트가 쌓여있는걸 볼 수 있습니다.  
![image](https://user-images.githubusercontent.com/105098581/223003843-f639f775-6e09-4b96-a826-1ca2d676a072.png)  
최초로 생기는 실행 컨텍스트는 전역에서 접근가능한 global excute context라고 부릅니다.  
함수를 위해서 만들어진 실행 컨텍스트는 function excute context라고 부릅니다.   
  
실행 컨텍스트는 **실행 컨텍스트 스택**과 **렉시컬 환경**으로 구성되어 있습니다.
- 실행 컨텍스트 스택은 코드의 실행 순서를 관리하는 자료구조로, L.I.F.O(Last In Out)구조로 들어오는 코드를 관리합니다.
- 렉시컬 환경은 모든 식별자와 바인딩된 값, 스코프를 기록 및 관리하는 자료구조입니다.
- 렉시컬 환경은 키와 값을 갖는 객체 형태의 스코프(전역, 함수, 블록 스코프)를 생성하여 식별자를 키로 등록하고 식별자에 바인딩된 값을 관리합니다.
- 각 실행 컨텍스트마다 렉시컬 환경이 존재합니다.
